...
import { create as createPersona, update as updatePersona } from '@/modules/seguridad/personas/services'
import { findCombo as findTiposIdCombo } from '@/modules/maestras/tipos-identificacion/services'
import { findCombo as findGenerosCombo } from '@/modules/maestras/generos/services'
import { findCombo as findPaisesCombo } from '@/modules/maestras/paises/services'
import { findDepartamentos, findProvincias, findDistritos } from '@/modules/maestras/ubigeos/services'
...
export default function PersonaForm({ open, onOpenChange, editing }: Props) {
  ...
  useEffect(() => {
    if (editing) {
      setForm({
        tipoIdentificacionId: editing.tipoIdentificacionId,
        numeroDocumento: editing.numeroDocumento,
        correoPersonal: editing.correoPersonal,
        nombres: editing.nombres,
        apellidoPaterno: editing.apellidoPaterno || '',
        apellidoMaterno: editing.apellidoMaterno || '',
        fecNac: editing.fecNac || '',
        numCelular: editing.numCelular || '',
        generoId: editing.generoId,
        ubigeoId: editing.ubigeoId || '',
        paisId: editing.paisId || 0,
        direccion: editing.direccion || '',
      })
      if (editing.ubigeo) {
        setDepartamento(editing.ubigeo.departamento || '')
        setProvincia(editing.ubigeo.provincia || '')
      }
    } else {
      setForm({ ...emptyForm })
      setDepartamento('')
