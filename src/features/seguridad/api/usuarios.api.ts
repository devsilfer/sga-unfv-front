...
import type { Usuario, CreateUsuarioInput, UpdateUsuarioInput } from '../types/usuario.types'
...
export async function create(input: CreateUsuarioInput): Promise<Usuario> {
  const { data } = await apiClient.post<Usuario>('/usuarios', input)
  return data
}

export async function update(id: number, input: UpdateUsuarioInput): Promise<Usuario> {
